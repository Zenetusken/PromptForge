...
- **Context enrichment**: unified `ContextEnrichmentService.enrich()` for all routing tiers. Heuristic analysis runs for ALL tiers (domain detection for cross-domain filtering). Three-layer codebase context: cached explore synthesis (background Haiku, once per repo link/reindex) + per-prompt curated retrieval (full source delivery, 80K char cap, import-graph + doc-ref expansion, source-type balance, skip-and-continue packing, 5-min TTL) + performance signals (strategy perf by domain, anti-patterns, domain keywords). Test files excluded from index. Incremental refresh (`REPO_INDEX_REFRESH_INTERVAL`, default 600s) detects changed files via HEAD SHA comparison. Zero request-time LLM calls for context

...
- **Feedback adaptation**: strategy affinity counter. Degenerate pattern detection (>90% same rating over 10+ feedbacks). Phase weight adaptation: context-derived initialization (`resolve_contextual_weights()` — task-type bias + cluster learned profiles), warm-path score-correlated adaptation (z-score weighting, EMA alpha=0.05) at both global and per-cluster level, decay toward learned weights (rate=0.01). Three closed feedback loops: weight learning, few-shot quality, cluster lifecycle

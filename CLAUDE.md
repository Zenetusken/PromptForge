...
- **Structured output via tool calling**: sends Pydantic-derived `Tool` schemas via `tools` + `tool_choice` on `create_message()`. Falls back to text parsing with JSON schema injection when client doesn't support tools (catches `McpError`). Scoring capped at 1024 tokens with JSON terminal directive. Text fallback includes `_strip_meta_header` (preamble/fence/orphan removal), `_split_prompt_and_changes` (14 marker patterns), `_build_analysis_from_text` (keyword-based classification from raw prompt).

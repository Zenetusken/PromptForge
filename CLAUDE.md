...
- **Feedback adaptation**: strategy affinity counter. Degenerate pattern detection (>90% same rating over 10+ feedbacks). Phase weight adaptation: warm-path batch computation identifies weight profiles correlated with highest `overall_score` values via z-score weighting, adapts toward them via EMA (alpha=0.05); warm-path decay (rate=0.01) drifts back to defaults

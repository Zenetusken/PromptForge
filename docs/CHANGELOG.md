...
### Added
- **Cancel button during pipeline** — SYNTHESIZE button becomes CANCEL (neon-yellow accent) during analyzing/optimizing/scoring phases. Secondary cancel link in Inspector phase indicator. Provides a way to abort stuck or unwanted pipeline runs
- **Elapsed timer in StatusBar** — shows seconds elapsed next to phase progress (e.g., `analyzing [1/3] 12s`) during active pipeline execution

### Changed
- **Intelligence layer principle in optimizer** — rewrote codebase context guidance in `optimize.md` and `agent-guidance.md` from passive "do NOT enumerate" caveat to first-class principle with good/bad examples. Optimizer now instructed to use codebase knowledge to formulate prompts the way an informed developer would (correct vocabulary, awareness of real risks) rather than converting it into scope-prescriptive checklists of files and functions. New "Scope Prescription" anti-pattern added to `agent-guidance.md` alongside existing calibration example in `scoring.md` that already penalizes file enumeration
- **Sampling degradation toast** — when the web UI routes to passthrough/internal because sampling requires the IDE, a toast now explains: "Sampling unavailable from browser -- using passthrough"
- **Rebalanced scoring dimension weights** — conciseness raised from 0.10 to 0.20 (was half the weight of any other dimension, creating a mathematical incentive to expand prompts). New weights: clarity 0.20, specificity 0.20, structure 0.15, faithfulness 0.25, conciseness 0.20. DRY: all 3 pipeline files now import `DIMENSION_WEIGHTS` from `score_blender.py` instead of hardcoding
- **Context anchoring softened** — optimizer no longer told to "always anchor to codebase context even if generic." Now instructs to sharpen language using context without enumerating file paths and architecture details the executor can discover independently
- **Structure proportional to complexity** — optimizer instruction changed from "add ## headers" (unconditional) to "add structure proportional to complexity" with explicit guidance that simple prompts stay flat
- **Length constraint hardened** — ±50% guidance upgraded from suggestion to hard constraint with explicit "3x is padding, 10x is always wrong" anchors. Prefer precision (better words) over accretion (more words)
- **Anti-verbosity patterns expanded** — added 3 new anti-patterns: enumerating discoverable file paths, restating user's scope in a separate section, prescribing rigid output templates for open-ended requests
- **Scoring conciseness rubric sharpened** — changed from "length is not a flaw" to "judge compression ratio: could the same information be conveyed in fewer words?" with explicit penalization of unnecessary length

### Fixed
- **Cross-process event forwarding silent drops** — 5 failure points in the MCP→backend→SSE taxonomy event delivery chain diagnosed and fixed: (1) sync fallback via daemon thread when no asyncio event loop is running (events were JSONL-only), (2) lazy-init of backend ring buffer when `taxonomy_activity` arrives before lifespan completes (events published to SSE but missing from `/api/clusters/activity`), (3) bounded retry queue (50 events) with drain-on-success for failed HTTP POSTs, (4) replay buffer increased from 200→500 to survive warm-path bursts during SSE reconnection gaps, (5) consecutive dedup changed from per-key dict to single-previous-event tracking so non-consecutive identical events (e.g. same Q scores across different phases) are no longer falsely suppressed
- **Taxonomy activity events intermittently missing from Activity panel** — Two root causes: (1) Backend forwarding chain had 5 silent failure points — `taxonomy_activity` was non-critical (zero retries), fire-and-forget tasks with no shutdown drain, 5s timeout too short for warm-path, all failures at debug level, ring buffer mirror swallowed exceptions. Fixed: retry + 15s timeout + task tracking + warn-level logging. (2) Frontend `EventSource` never registered listeners for `taxonomy_activity`, `seed_batch_progress`, `optimization_status`, `optimization_score_card`, or `optimization_start` — named SSE events silently dropped by browser. Fixed `connectEventStream()` to include all 16 event types. Also fixed double-prefix bug (`optimization_optimization_start` → `optimization_start`) in MCP tool forwarding and wired `agent_changed` for seed agent hot-reload
- **Leaked MetaPatterns polluting cross-cluster injection** — 85% of meta-patterns (2058/2432) belonged to archived clusters, inflating `global_source_count` to saturation (max 20 everywhere) and injecting dead-cluster patterns into the optimizer. Root cause: `split_cluster()` archived parents without deleting their MetaPatterns. Fix: (1) clean up parent patterns on split, (2) backfill cleanup in warm-path reconcile, (3) archived-state filter added to `global_source_count` computation and all 3 downstream consumers (`pattern_injection`, `fusion`, `matching`)
- **Snapshot table unbounded growth** — 18,916 TaxonomySnapshot rows accumulated because the existing `prune_snapshots()` retention function was never called. Wired into warm-path after Phase 6 audit commit. Added `created_at DESC` index for query performance
- **Semi-orphan optimizations accumulating between restarts** — optimizations pointing to archived clusters (from split/merge/retire) now reassigned to nearest active cluster during warm-path reconcile, capped at 50 per cycle. Stale `OptimizationPattern` join records also cleaned
- **Phase 4 refresh per-cluster error handling** — a single cluster failure (e.g. SQLite write contention) in Phase D no longer aborts the entire refresh pass; remaining stale clusters still get processed
- **Score-weight formula mismatch across taxonomy paths** — hot-path cluster assignment used power-law centroid weighting (`(score/10)^1.5`) while warm-path reconciliation and cold-path refit used linear (`score/10`), causing every reconciliation cycle to silently rewrite centroids with weaker differentiation. Extracted shared `score_to_centroid_weight()` helper and unified all 4 call sites (hot merge, new cluster, warm reconcile, cold reconcile) on the power-law formula
- **Merge centroid weighted by count instead of score-weight** — `attempt_merge()` blended two centroids using `member_count` instead of `weighted_member_sum`, under-weighting clusters whose members had higher scores. Also failed to update `weighted_member_sum` on the survivor and zero it on the loser. Fixed merge to use `weighted_member_sum` for centroid blending and propagate it correctly. Added `weighted_member_sum = 0.0` to all archive paths (merge loser, retire, dissolve, split parent) to prevent phantom data on archived clusters
- **Sampling proxy hang from web UI** — removed broken MCP sampling proxy that hung at "analyzing [1/3]" for up to 8 minutes when Force IDE Sampling was ON and user clicked Synthesize in the browser. The proxy opened a non-sampling MCP session, so `create_message()` targeted the proxy (not the IDE) and timed out per-phase. Requests now correctly degrade to passthrough/internal with clear feedback

### Removed
- **Dead sampling proxy recovery code** — removed 15-second setTimeout branches in forge store error/close callbacks that only fired for the now-removed proxy path

...
### Fixed
- **Template cluster visibility in non-template state filters** — template clusters (e.g. the JWT Token Lifestyle spawn) rendered in the "active" tab as a labeled-less cyan sphere whose hover chip fell back to the domain name (`SECURITY`) because `stateOpacity()` ghosted non-matching states to 0.25, which tripped the `nodeOpacity < 0.5` branch that blanks `SceneNode.label` for LOD/ghost cleanup. Templates are architecturally structural (spawned from mature clusters, cross-filter surfaces) so they join `domain`/`project` as semi-visible structural nodes at 0.5 in filtered tabs. `SemanticTopology.svelte`'s template-sprite force-visible pass now has non-empty text to render, and the hover chip displays the cluster label as intended. 2 new Vitest cases lock the structural-opacity floor and the matching-filter (`template` tab) still-at-1.0 invariant.
- **Pattern graph sub-domain color parity** — sub-domain nodes in `SemanticTopology` (state=domain with a domain parent) used to resolve colors via their own label (e.g. `token-ops` → backend-generated OKLab variant `#d20033` dark red) instead of inheriting the parent domain's canonical brand color (`security` → `#ff2255`). `TopologyData.buildSceneData()` now walks up the `state="domain"` parent chain via `rootDomainLabel()` (cycle-guarded, 8-hop cap) and resolves color against the top-level domain's label. `ClusterNavigator.svelte` `SubDomainGroup` extended with `parentLabel` so the sub-domain row's color dot matches the pattern graph's sphere. 3 new Vitest cases lock sub-domain inheritance, top-level non-regression, and cluster-under-sub-domain top-level inheritance.
- **`PATCH /api/preferences` 422 on readiness toggles** — the router's `PreferencesUpdate` Pydantic model sets `extra="forbid"` but never declared the `domain_readiness_notifications` key shipped in PR #27, so every per-row mute and the new master-bell click returned 422 before reaching the service layer (which *did* know about the key). Fix adds a strict `_DomainReadinessNotificationsUpdate` sub-model (`enabled: StrictBool | None`, `muted_domain_ids: list[str] | None`, `extra="forbid"` for sub-forbidding). 7 regression tests lock partial-toggle-only, partial-list-only, both-together, unknown sub-key rejection, non-bool rejection, non-string list entries, and top-level strictness invariant

...
Complete inventory of all 74 Svelte 5 components (68 in `components/`, 6 in `components/ui/`). Components are organized into **shared** (reusable across the app) and **individual** (single-use, feature-specific) categories.
...
Barrel-exported from `components/ui/index.ts`. Import as `{ Tooltip, MetaBadge } from "./ui"`.
...
| `DesktopWindow` | `windowId`, `title`, `icon`, `minimizable?`, `onclose?` | `windowManager` | Window chrome: title bar with minimize/maximize/close, z-index stacking. Address bar rendered between title bar and content when breadcrumbs or navigation state exists. Back/forward chevron buttons appear when `windowManager.getNavigation(windowId)` is set, with 1px vertical divider separating nav buttons from breadcrumb path. Drag/resize safety reset: `blur`/`visibilitychange` listeners clear stuck `dragging`/`resizing` state if `mouseup` is missed (mouse leaves viewport, tab loses focus). |
...
| `TaskbarWindowButton` | `win: WindowEntry` | `windowManager`, `forgeMachine` | Individual window button in the taskbar. Active click: minimizes persistent windows (`PERSISTENT_WINDOW_IDS`), navigates home for route-driven windows. Pulsing dot for IDE when forge is running. |
...
| `ControlPanelWindow` | — | `settingsState`, `providerState`, `processScheduler` | System settings window with 4 tabs: Providers (active provider, model, connection status from `providerState.health`), Pipeline (default strategy selector from `ALL_STRATEGIES`, max concurrent forges 1–5 synced to `processScheduler.maxConcurrent`, auto-retry on rate limit toggle), Display (10-color accent swatch grid from `NEON_COLORS`, animations toggle), System (backend version, DB/MCP connection status, active process counts, Reset All Settings button). |

...
| `MoveToDialog` | `open` (bindable), `nodeType: 'project'\|'prompt'\|null`, `nodeId: string\|null`, `onmove?`, `oncancel?` | `fsOrchestrator` | Folder picker modal for "Move to..." context menu action. Loads root-level folders via `fsOrchestrator.loadChildren(null)`, excludes self when moving a folder. No pre-selection; Move button disabled until a folder is chosen. Uses `AlertDialog` from bits-ui. |

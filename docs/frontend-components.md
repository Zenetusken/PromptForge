...
| `ForgeIDEExplorer` | `onjumpline?` | `forgeSession`, `forgeMachine`, `projectsState`, `promptAnalysis` | Left pane (resizable, 180–360px via `forgeMachine.explorerWidth`): Metadata, Outline, Analysis, and Context collapsible sections. **Outline** shows detected prompt sections (colored dot + truncated label + `:lineNumber`, clickable via `onjumpline`) and variable count. **Analysis** shows heuristic task type with confidence %, matched keyword chips, and recommended strategies with composite scores. Resize handle on right edge. Header row with collapse-all button (includes `showOutline`/`showAnalysis`). |
| `ForgeIDEEditor` | — | `forgeSession`, `optimizationState`, `forgeMachine`, `promptAnalysis`, `processScheduler`, `tabCoherence` | Center pane (flex-1): multi-tab workspace with tab bar, breadcrumb bar (`project / title.ext`), renders `ForgeEditor` per tab. Auto-focuses editor on IDE entry and tab switch (compose mode only). Document-aware tab icons: cyan `file-text` for prompts, purple `zap` for artifacts, gray `file-text` for untitled tabs (via `tabIconName()`/`tabIconColor()`). Modified indicator (`·`) on tabs when `draft.text !== originalText` (morphs to `×` on hover). Tab bar is a drop target for `DragPayload` — accepts drops from HistoryWindow and ProjectsWindow rows. **Breadcrumb bar**: document icon (`file-text`), cursor-aware section breadcrumb (colored by `SECTION_COLORS`), sourceAction badge ("Optimizing"/"Reiterating"), version badge (green border), first 2 tags as indigo pills, linked prompt indicator (`link` icon), context status indicator (`server` icon with tooltip). **Validation error bar**: dismissible red bar between breadcrumb and editor showing `forgeSession.validationErrors`. **Forging toolbar**: shows current pipeline stage label (uppercase), iteration counter (`iter N/3`), concurrent forge count, error state with retry countdown; replaces static "Forging..." text. **Status bar**: `Ln {line}`, word count, char count (from centralized `forgeSession.wordCount`/`charCount`), selection count (neon-cyan when > 0), variable count, analysis spinner (orange pulse when debounce pending), task type badge wrapped in keywords tooltip (`matchedKeywords`), strategy quick-pick popover (top 3 `recommendedStrategies` with composite scores, click-outside dismiss), keyboard shortcut hint. Exports `jumpToLine()` (delegates to `ForgeEditor`). Tab operations (`switchTab`, `closeTab`, `newTab`, `exitIDE`) use `saveActiveTabState()`/`restoreTabState()` from `tabCoherence.ts`. Forging guard blocks tab switching, active-tab close button (`pointer-events-none`), and new tab creation during an active forge. |

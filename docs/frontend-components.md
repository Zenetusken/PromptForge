...
| `AuditLogWindow` | — | `systemBus`, `auditClient` | Kernel audit log viewer. Table with columns: Timestamp (relative: "just now", "5m ago"), App (dropdown filter), Action (color-coded: optimize=cyan, transform=green, delete=red, create=purple), Resource type + ID. Pagination with prev/next buttons. Live-updates via `kernel:audit_logged` bus event subscription. |

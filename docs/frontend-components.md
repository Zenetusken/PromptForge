...
| `FolderWindow` | — | `fsOrchestrator`, `windowManager`, `toastState` | Hierarchical folder browser rendered inside `DesktopWindow`. Breadcrumb navigation via `fsOrchestrator.getPath()`. Shows folders and prompts with `.md` extension appended to prompt names. Forge count badge on prompts with `forge_count > 0` (neon-purple). Expandable forge children: disclosure chevron on prompt rows, `expandedPrompts: Set<string>` state, fetches forge list on expand via `GET /api/projects/{id}/prompts/{pid}/forges`, renders each forge as indented row with purple `zap` icon and `.forge` extension. Double-click forge → `openDocument(createArtifactDescriptor(...))`. Drag-and-drop support for moving items between folders. |
...
| `TaskManagerWindow` | — | `processScheduler`, `providerState`, `mcpActivityFeed` | Process monitor with sortable table (PID, Title, Status, Strategy, Progress, Duration). Summary bar: running/queued/completed counts with color dots, LLM provider status. Per-process actions: Cancel (running/queued), Open in IDE (completed, via `openDocument(createArtifactDescriptor(...))`), Retry (errored), Dismiss (non-running). Progress column: running shows percentage bar, completed shows score, error shows truncated message. External (MCP) section shows active MCP tool calls with progress. Footer: max concurrent setting, token usage, next queued PID. |

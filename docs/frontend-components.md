...
| `WorkspaceWindow` | — | `workspaceManager`, `providerState`, `windowManager`, `desktopStore` | Workspace Hub with 3 tabs: **GitHub** (OAuth setup form / connected state with repo list + project linking), **Workspaces** (linked workspace cards with sync status dots, sync/unlink actions), **Context Inspector** (per-field provenance display — loads kernel profile via `knowledge.getProfile()`, shows manual/auto/n/a badges with neon-purple/green/dim colors; auto value shown as fallback when manual overrides exist). Status bar shows GitHub connection + link/sync/error counts from provider health. |
...
| `ForgeContextSection` | `compact?` | `forgeSession`, `projectsState` | IDE explorer project context editor with stack template picker. Auto-populates from project profile. Shows source count badge when project has knowledge sources. Pre-forge context preview button resolves and displays full context via `ContextSnapshotPanel`. |

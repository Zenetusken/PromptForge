...
| `FolderWindow` | `folderId` | `fsOrchestrator`, `windowManager`, `systemBus` | Hierarchical folder browser rendered inside `DesktopWindow`. Breadcrumb navigation via `fsOrchestrator.getPath()`. Shows folders and prompts with `.md` extension appended to prompt names. Forge count badge on prompts with `forge_count > 0` (neon-purple). Expandable forge children: disclosure chevron on prompt rows, `expandedPrompts: Set<string>` state, fetches forge list on expand. Double-click forge → `openDocument(createArtifactDescriptor(...))`. Drag-and-drop: folder/prompt rows emit `DragPayload` with `source: 'folder-window'`; content area and folder rows accept drops from any `DragSource` (including `'desktop'`) via `DRAG_MIME` → `fsOrchestrator.move()`. Context menu: single-item (Open / Move to... / Delete) and batch (Move N items to... / Delete N items) with `Ctrl/Cmd+click` multi-selection. Local `MoveToDialog` and `ConfirmModal` instances. Subscribes to `fs:created/moved/deleted/renamed` bus events for live reload. Also subscribes to `forge:completed` — when a forge completes for a prompt in this folder (matched by `projectId === folderId`), clears the prompt's cached forge list and reloads contents to update forge count badges. |
| `WorkspaceWindow` | — | `workspaceManager`, `providerState`, `windowManager`, `desktopStore` | Workspace Hub with 3 tabs: **GitHub** (OAuth setup form / connected state with repo list + project linking), **Workspaces** (linked workspace cards with sync status dots, sync/unlink actions), **Context Inspector** (fully editable knowledge profile editor — loads kernel profile via `knowledge.getProfile()`, unpacks into editable `$state` fields; identity fields (language, framework, description, test_framework) save on blur via `saveInspectorIdentity`, hint fields (conventions, patterns, test_patterns) save as newline-split arrays via `saveInspectorHint`; completeness bar computed from kernel profile field count via `inspectorCompleteness` derived; `SourceManager` embedded for knowledge source CRUD; "auto" badge on empty fields when auto-detected value exists). Status bar shows GitHub connection + link/sync/error counts from provider health. |
...
| `ForgeIDEExplorer` | `onjumpline?` | `forgeSession`, `forgeMachine`, `projectsState`, `promptAnalysis` | Left pane (resizable, 180–360px via `forgeMachine.explorerWidth`): Metadata, Anatomy, Analysis, and Context collapsible sections. **Anatomy** renders `PromptAnatomyHUD` in compose mode — shows detected prompt sections (colored 2px left border + type badge + label + `:lineNumber`, clickable via `onjumpline`) and variable chips with occurrence counts. Replaces former "Outline" section. **Analysis** shows heuristic task type with confidence %, matched keyword chips, and recommended strategies with composite scores. Resize handle on right edge. Header row with collapse-all button (includes `showOutline`/`showAnalysis`). |
| `ForgeIDEEditor` | — | `forgeSession`, `optimizationState`, `forgeMachine`, `promptAnalysis`, `processScheduler`, `tabCoherence` | Center pane (flex-1): multi-tab workspace with tab bar, breadcrumb bar (`project / title.ext`), renders `ForgeEditor` per tab. Auto-focuses editor on IDE entry and tab switch (compose mode only). Document-aware tab icons: cyan `file-text` for prompts, purple `zap` for artifacts, gray `file-text` for untitled tabs (via `tabIconName()`/`tabIconColor()`). Modified indicator (`·`) on tabs when `draft.text !== originalText` (morphs to `×` on hover). Tab bar is a drop target for `DragPayload` — accepts drops from HistoryWindow and ProjectsWindow rows. **Breadcrumb bar**: document icon (`file-text`), cursor-aware section breadcrumb (colored by `SECTION_COLORS`), sourceAction badge ("Optimizing"/"Reiterating"), version badge (green border), first 2 tags as indigo pills, linked prompt indicator (`link` icon), save button (Ctrl+S via `forge:save` bus event, green flash, requires project), context status popover (`server` icon — shows source/language/framework/size, Clear/Configure actions, click-outside dismiss). **Validation error bar**: dismissible red bar between breadcrumb and editor showing `forgeSession.validationErrors`. **Analysis inline bar**: task type + complexity `MetaBadge`, weakness/strength counts, analysis duration; dismissible, shown in compose mode when `analysisResult` present. **Forging toolbar**: shows current pipeline stage label (uppercase), iteration counter (`iter N/3`), concurrent forge count, error state with retry countdown; replaces static "Forging..." text. **Compose toolbar**: auto-refine toggle with min score slider, session context checkbox, Analyze Only button (`runNodeAnalyze` with spinner), Tournament strategy picker. **Review toolbar**: Re-forge/Chain/Iterate via shared `forgeActions.ts` (`reforge()`, `chainForge()`, `iterate()`), score badge, Back to Compose button. **Status bar**: `Ln {line}`, word count, char count (from centralized `forgeSession.wordCount`/`charCount`), selection count (neon-cyan when > 0), variable count, analysis spinner (orange pulse when debounce pending), task type badge wrapped in keywords tooltip (`matchedKeywords`), strategy quick-pick popover (top 3 `recommendedStrategies` with composite scores + secondary strategy toggles delegating to `forgeSession.toggleSecondaryStrategy()`; click-outside dismiss via shared `createDismissEffect` helper), secondary strategies badge (+N count), default strategy indicator (`(default)` label when strategy matches `settingsState.defaultStrategy`), keyboard shortcut hint. Exports `jumpToLine()` (delegates to `ForgeEditor`). Tab operations (`switchTab`, `closeTab`, `newTab`, `exitIDE`) use `saveActiveTabState()`/`restoreTabState()` from `tabCoherence.ts`. Forging guard blocks tab switching, active-tab close button (`pointer-events-none`), and new tab creation during an active forge. |
| `ForgeIDEInspector` | — | `promptAnalysis`, `forgeSession`, `optimizationState`, `forgeMachine`, `windowManager` | Right pane (resizable, 280–560px via `forgeMachine.panelWidth`): strategy selector with heuristic recommendations (analyzing spinner, "type more" hint), Analyze Only / Full Optimization buttons, pipeline step inspector, score display, error panel. Full Optimization delegates to `optimizationState.startOptimization()` which internally spawns a process via `processScheduler`. |
| `ForgeEditor` | `variant?: "collapsed"\|"focus"`, `oncursorchange?`, `onselectionchange?`, `onsubmit?`, `onexitfocus?` | `forgeSession`, `optimizationState`, `toastState`, `promptAnalysis` | Prompt textarea with slash-command autocomplete (`/strategy`), visual line numbers gutter (focus mode, scroll-synced) using hidden mirror div measurement — counts visual wrapped lines, not just logical `\n` lines. Structure section dots (merged into gutter as colored left borders from `promptAnalysis.sections`, mapped from logical to visual line positions). Cursor position tracking reports visual line (`oncursorchange`), selection tracking (`onselectionchange`), variable chips with occurrence cycling (from `promptAnalysis.variables` — clicking cycles through all occurrences with scroll-to-line, shows `current/total` counter). ResizeObserver triggers re-measurement on container resize. Exports `jumpToLine(lineNumber)`, `focus()`. Uses `promptParser.ts` types. |
...
| Component | Props | Stores | Description |
|-----------|-------|--------|-------------|
| `ForgeReview` | — | `optimizationState`, `forgeMachine`, `forgeSession` | IDE review panel: score header, dimension bars, optimized/original tabs, strategy reasoning, **Prompt Anatomy** collapsible panel (renders `PromptAnatomyHUD` in review mode with pipeline-detected sections/variables from `result.detected_sections`/`result.detected_variables`), iteration timeline, actions (Copy/Iterate/Re-forge/Compare). |
...
| `ForgeContents` | `result: OptimizationResultState` | None | Collapsible sub-artifact accordion in IDE review mode. 4 virtual files: `analysis.scan` (task type, complexity, strengths, weaknesses, changes made), `scores.val` (5 dimension score bars + overall + verdict + improvement indicator), `strategy.strat` (strategy name, confidence %, secondary frameworks, reasoning), `anatomy.anat` (delegates to `PromptAnatomyHUD mode="review"`; conditionally hidden when no structure data). Each row has extension-colored icon and filename with extension. Chevron toggle expands inline content. Dynamic file count label. |
| `PromptAnatomyHUD` | `sections`, `variables`, `mode`, `onjumpline?`, `totalLines?` | None | Consolidated prompt structure panel. Accepts data from `promptAnalysis` store (compose mode) or `OptimizationResultState` (review mode). Section rows: 2px left border colored by `SECTION_COLORS`, uppercase type badge, label, line number. Variable chips: `{{name}} ×N` in neon-teal. Summary bar with section/variable counts. Coverage bar (compose mode only, when `totalLines` provided). Compose mode: `transition:slide` entrance, clickable rows fire `onjumpline(lineNumber)`. Review mode: staggered `stagger-fade-in` animation. Respects `prefers-reduced-motion`. |
...
| Component | Props | Stores | Description |
|-----------|-------|--------|-------------|
| `ForgeMetadataSection` | `projectListId?`, `compact?` | `forgeSession`, `projectsState` | IDE explorer metadata: title, project selector, tags, version fields. Each field has an icon + uppercase label (edit/git-branch/folder/layers). Project field shows sourceAction badge (OPT/RET) and linked prompt indicator (`link` icon) when applicable. |
| `ForgeContextSection` | — | `forgeSession`, `projectsState`, `knowledge` | IDE explorer project context editor. Flat single-level layout: identity badges (language/framework from kernel, shown only when kernel profile has values — source count accompanies badges but doesn't appear alone), editable fields (conventions, patterns, test framework, test patterns) always visible, `<select>` dropdown for stack templates, Clear button. Auto-populates from project profile. Auto-fetching resolved summary (debounced 600ms) shows field count, char estimate, and compact one-line digest using `shortLabel()` truncation at natural breaks. Trigger badge shows `N fields · XK`; uses shared `formatChars()` from `utils/safe.ts`. |
...
| `ContextSnapshotPanel` | `context: CodebaseContext` | None | Collapsible read-only display of resolved project context snapshot attached to an optimization. Shows field count badge, language/framework/test_framework as neon-cyan badges, conventions/patterns/test_patterns as pill lists, code snippets as code blocks, documentation truncated to 500 chars, knowledge sources as expandable tagged list. Used in `ForgeReview`. |
| `SourceManager` | `appId: string`, `entityId: string`, `projectStatus?: string` | None | CRUD manager for project knowledge sources (NotebookLM-style). Add/edit/delete/toggle sources with type badges, budget bar (chars/100K), inline editor, expand-in-place content preview (click title to toggle, 2K char truncation). Read-only when project is archived. Rendered in `ProjectsWindow` folder view at project root level. |

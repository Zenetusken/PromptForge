<script lang="ts">
  ...
  let chips = $state<{ id: string; label: string; type: string }[]>([]);
  ...
  function addChip(type: string) {
    const label = type === 'repo' && github.selectedRepo
      ? github.selectedRepo
      : `@${type}`;
    chips = [...chips, { id: `ctx-${Date.now()}`, label, type }];
    showMenu = false;
  }

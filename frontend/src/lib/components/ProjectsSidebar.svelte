	...
	<div class="flex items-center gap-1.5">
		<div class="relative flex-1">
			...
			<input
				...
				class="input-field w-full py-1.5 pl-8 pr-2 text-[12px]"
			/>
		</div>
		{#if !isArchived}
			<Tooltip text="Create new project" side="bottom" class="shrink-0">
				<button
					...
					class="flex h-6 w-6 items-center justify-center rounded-md border border-neon-cyan/15 text-neon-cyan/60
	...
	<SidebarTabs
		...
		tabs={[
			{ value: "active", label: "Active", testid: "filter-active" },
			{
				...
				activeClass: "text-neon-yellow bg-neon-yellow/8",
			},
		]}
	/>
</div>
...
<div class="flex-1 overflow-y-auto px-1.5 pb-1.5" data-testid="projects-list">
	{#if projectsState.isLoading && !projectsState.hasLoaded}
		<div class="space-y-1.5 p-1.5" data-testid="projects-skeleton">
			{#each [1, 2, 3] as _}
				<div class="rounded-lg p-1.5">
					<div class="mb-1 flex items-center gap-1.5">
						<div
							class="skeleton h-3.5 w-3.5 shrink-0 rounded"
						></div>
						<div class="skeleton h-3.5 w-3/5"></div>
						<div
							class="skeleton ml-auto h-4 w-5 shrink-0 rounded-full"
						></div>
					</div>
	...
	{:else if projectsState.items.length === 0}
		...
		>
			<div class="mb-2 rounded-xl bg-bg-hover/50 p-2.5 text-text-dim">
			...
			{:else if isArchived}
				<p class="mt-0.5 text-[11px] leading-relaxed text-text-dim">
					Archived projects will appear here
				</p>
			{:else}
				<p class="mt-0.5 text-[11px] leading-relaxed text-text-dim">
					Create one to organize your prompts
				</p>
			{/if}
		</div>
	{:else}
				...
				<button
					...
					onclick={() =>
						projectsState.loadProjects({
							page: projectsState.page + 1,
						})}
					data-testid="projects-load-more"
				>
					Load more ({projectsState.total -
						projectsState.items.length} remaining)
				</button>
			</div>
		{/if}
	{/if}

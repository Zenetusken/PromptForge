<script lang="ts">
	...
	let currentIndex = $derived(
		forges.findIndex((f) => f.id === currentForgeId),
	);
	let prevForge = $derived(
		currentIndex > 0 ? forges[currentIndex - 1] : null,
	);
	let nextForge = $derived(
		currentIndex < forges.length - 1 ? forges[currentIndex + 1] : null,
	);
			...
			<div class="flex items-center gap-2">
				...
				{:else}
					<Tooltip text="No previous results">
						<span
							class="flex items-center gap-1 rounded-lg px-2 py-1 text-xs text-text-dim/30"
						>
							<Icon name="chevron-left" size={12} />
							Prev
						</span>
			...
			{#each forges as forge, i (forge.id)}
				...
				>
					...
					>
						...
						{#if forge.title}
							<span class="min-w-0 truncate text-text-dim"
								>{forge.title}</span
							>
						{:else if forge.framework_applied}
							<MetaBadge
								type="strategy"
								value={forge.framework_applied}
								showTooltip={false}
								size="xs"
							/>
						{/if}

...
>
	<div class="flex h-8 items-center gap-1.5 border-b border-border-subtle px-1.5">
		<div class="flex-1">
			<SidebarTabs
		...
		<Tooltip text="Close sidebar" side="left">
			<button
				onclick={() => (open = false)}
				class="flex h-6 w-6 items-center justify-center rounded-md text-text-dim transition-[background-color,color] duration-200 hover:bg-bg-hover hover:text-neon-cyan"
	...
	{#if sidebarState.activeTab === 'history'}
		...
		{#if showClearConfirm}
			<div class="animate-fade-in mx-1.5 mb-1.5 rounded-lg border border-neon-red/15 bg-bg-card p-2" data-testid="clear-confirm-dialog">
				<div class="mb-1.5 flex items-center gap-2">
					<div class="flex h-5 w-5 items-center justify-center rounded-lg bg-neon-red/10">
						<Icon name="trash-2" size={11} class="text-neon-red" />
					</div>
					<p class="text-[11px] font-semibold text-neon-red">Delete all history?</p>
				</div>
				<p class="mb-2 text-[11px] leading-relaxed text-text-dim">This permanently removes all optimization records. This action cannot be undone.</p>
				<div class="flex gap-2">
					<button
						class="flex-1 rounded-lg bg-neon-red/15 py-1 text-[11px] font-medium text-neon-red transition-colors hover:bg-neon-red/25"
		...
		<div class="flex-1 overflow-y-auto px-1.5 pb-1.5" data-testid="history-list">
			{#if historyState.isLoading && !historyState.hasLoaded}
				<div class="space-y-1.5 p-1.5" data-testid="history-skeleton">
					{#each [1, 2, 3, 4, 5] as _}
						<div class="rounded-lg p-1.5">
							<div class="skeleton mb-1.5 h-4 w-4/5"></div>

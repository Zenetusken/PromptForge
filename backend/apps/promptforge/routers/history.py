...
@router.head("/history", include_in_schema=False)
...
@router.post("/history/bulk-delete", response_model=BulkDeleteResponse)
...
async def get_stats(
    ...
    stats = await get_stats_cached(project, db)
    response.headers["Cache-Control"] = "max-age=30"
    return StatsResponse(**stats)


@router.head("/history/stats", include_in_schema=False)

"""PromptForge-specific database migrations.

Extracted from the host app/database.py — these are PF-owned schema changes
(optimizations, projects, prompts, workspace tables) that run during the
PromptForge app lifecycle via ``run_migrations()``.
"""
...
_MIGRATIONS: list[str] = [
    ...
    "  redirect_uri TEXT NOT NULL DEFAULT 'http://localhost:8000/api/apps/promptforge/github/callback',"
...
async def rebuild_projects_table(conn) -> None:
...
async def rebuild_prompts_table(conn) -> None:
...
async def migrate_legacy_strategies(conn) -> None:
    """Normalize legacy strategy names in historical optimization records."""
    from apps.promptforge.constants import LEGACY_STRATEGY_ALIASES
...
async def migrate_legacy_projects(conn) -> None:
...
async def backfill_prompt_ids(conn) -> None:
...
async def backfill_missing_prompts(conn) -> None:
    ...
    for project_id, raw_prompt in orphans:
        ...
        max_orders[project_id] = next_order
        inserts.append(
            {
                "id": str(uuid.uuid4()),
                "content": raw_prompt,

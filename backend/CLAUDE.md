...
**Domain discovery**: `_propose_domains()` from coherent "general" sub-populations (≥3 members, ≥0.3 coherence, ≥60% consistent `domain_raw`). Five guardrails: color pinning, retire exemption, merge gate, coherence floor, split→candidates. Ceiling at 30. **Sub-domain discovery**: `_propose_sub_domains()` is signal-driven — extracts sub-qualifiers from `domain_raw` (e.g. "backend: auth") + `intent_label` keyword fallback, promotes consistent qualifiers to nested domain nodes (`state="domain"`, `parent_id=parent_domain_id`, label `{parent}-{qualifier}`) using adaptive consistency threshold `max(0.40, 0.60 - 0.004 * members)`. No HDBSCAN, no Haiku label generation.

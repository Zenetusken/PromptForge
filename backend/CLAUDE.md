...
- **Codebase context**: pre-computed in background on repo link/reindex. Three layers: cached Haiku synthesis (architectural overview in `RepoIndexMeta.explore_synthesis`) + per-prompt curated retrieval (full source delivery via `RepoFileIndex.content`, 80K char cap, import-graph + doc-ref expansion, source-type balance via `INDEX_CURATED_DOC_CAP_RATIO`, cross-domain filtering, skip-and-continue packing with `_MAX_BUDGET_SKIPS=5`, 5-min TTL) + performance signals (strategy perf by domain, anti-patterns, domain keywords via `resolve_performance_signals()`). Test files excluded from indexing (`_is_test_file()`). Source-type classifier (`_classify_source_type()`: code/docs/config) balances retrieval. Incremental refresh via `REPO_INDEX_REFRESH_INTERVAL` (default 600s, SHA diff, upsert via unique composite index). Heuristic analysis runs for ALL tiers (domain detection). Zero request-time LLM calls. All tiers receive identical context via `ContextEnrichmentService.enrich()`

...
- **Codebase context**: pre-computed in background on repo link/reindex. Three layers: cached Haiku synthesis (architectural overview in `RepoIndexMeta.explore_synthesis`) + per-prompt curated retrieval (full source delivery via `RepoFileIndex.content`, 80K char cap, import-graph expansion, cross-domain filtering, 5-min TTL) + performance signals (strategy perf by domain, anti-patterns, domain keywords via `resolve_performance_signals()`). Test files excluded from indexing (`_is_test_file()`). Heuristic analysis runs for ALL tiers (domain detection). Zero request-time LLM calls. All tiers receive identical context via `ContextEnrichmentService.enrich()`

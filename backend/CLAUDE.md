...
- **Context delivery**: all tiers receive profile-gated context via `ContextEnrichmentService.enrich()`. 4 context_sources keys: `codebase_context`, `strategy_intelligence`, `applied_patterns`, `heuristic_analysis`. Template render key is `"strategy_intelligence"` everywhere (not `"adaptation_state"`). Passthrough/refine tiers get full `auto_inject_patterns()` in enrichment; internal/sampling defer to pipeline (provenance recording needs `optimization_id`). Few-shot examples in ALL tiers including passthrough. `applied_pattern_texts` stored in `enrichment_meta` for post-optimization UI attribution. `classification_agreement` singleton tracks E1 heuristic vs LLM agreement + strategy intel hit rate
- **Template lifecycle**: `PromptLifecycleService` auto-promotes active→mature (members≥5, coherence≥0.7, score≥7.0) and mature→template (usage≥3, score≥7.5). PATCH endpoint validates manual template promotion (score≥6.0 + members/usage). Warm-path Phase 0 demotes degraded templates (score<5.5 or coherence<0.4) with 0.5-point hysteresis. Phase 4 recomputes `preferred_strategy` for templates. `_log_template_event()` for observability
- **Pattern matching**: `match_prompt()` uses raw embeddings (no composite fusion — ensures cross-process consistency between backend and MCP). Thresholds: family 0.55, cluster 0.45, candidate 0.65. LRU embedding cache (50 entries, 5-min TTL). Diagnostic logging on no-match (best scores, thresholds, fusion status). Match endpoint rate-limited (30/min) with max_length=8000

...
**Analysis**: `heuristic_analyzer.py` (zero-LLM classifier, 6-layer, adaptive keyword weights), `context_enrichment.py` (unified `enrich()` for all tiers → frozen `EnrichedContext`, enrichment profiles: code_aware/knowledge_work/cold_start, `resolve_strategy_intelligence()` merges perf signals + adaptation feedback, task-gated curated retrieval), `context_resolver.py` (per-source char caps, untrusted wrapping)

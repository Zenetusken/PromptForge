...
@router.get("/api/fs/children", response_model=FsChildrenResponse)
...
@router.get("/api/fs/path/{project_id}", response_model=FsPathResponse)
async def get_path(
    project_id: str,
    db: AsyncSession = Depends(get_db_readonly),
):
...
@router.get("/api/fs/prompt/{prompt_id}", response_model=FsNode)
...
@router.delete("/api/fs/prompt/{prompt_id}")
...
@router.post("/api/fs/move", response_model=MoveResponse)
async def move_node(
    body: MoveRequest,
    db: AsyncSession = Depends(get_db),
):

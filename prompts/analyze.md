...
3. **Domain** — Use the format "primary: qualifier" where the primary domain is one of: backend, frontend, database, devops, security, fullstack, general. The qualifier adds specificity about the cross-cutting concern or sub-area (e.g., "backend: auth middleware", "frontend: accessibility", "backend: security", "database: migration"). If no qualifier applies, use just the primary domain (e.g., "backend"). Use "general" only for non-development prompts.

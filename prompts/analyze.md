...
3. **Domain** — Use the format "primary: qualifier" where the primary domain is one of: {{known_domains}}. The qualifier adds specificity about the cross-cutting concern or sub-area (e.g., "backend: auth middleware", "frontend: accessibility"). If no qualifier applies, use just the primary domain. Prefer the most specific domain that matches the prompt's primary concern — "backend: database" is better than "fullstack" for a database-focused prompt. Use "fullstack" only when the prompt equally emphasizes BOTH frontend AND backend (e.g., "build a React form that submits to a FastAPI endpoint"). Use "general" only for prompts that don't fit any specific domain.
